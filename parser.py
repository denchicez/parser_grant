...
def get_html(url, params=None):  # делаем запрос на html страничку
    ...
    try:
        r = requests.get(url, headers=HEADERS, params=params,
                         timeout=60)  # делаем запрос на страницу максимальный отклик 60секунд
        return r
    except:
...
def NameCheck(string, code1, code2):  # Можно ли расшифровать stirng с помощью code1 и code2
    letters = ['“', '…', '”', '<', '>', '«', '»', chr(9), chr(13), chr(10), '(', ')', '|', ':', ' ', chr(34), 'a', 'b',
               'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w',
               'x', 'y', 'z', 'а', 'a', 'б', 'в', 'г', 'д', 'е', 'ё', 'ж', 'з', 'и', 'й', 'к', 'л', 'м', 'н', 'о', 'п',
...
def get_social_links(links):  # ищем из всех ссылок ссылки на соц. сети
    prefixes = [['https://www.youtube.com/channel/', 'http://www.youtube.com/', 'https://www.youtube.com/user/',
                 'https://www.youtube.com/c/'], ['https://vk.com/'], ['https://www.instagram.com/']]
    youtube = []
    youtube_links = set()
    vk = []
    vk_links = set()
...
def file_saving():  # cохраняем файл
    ...
    with open(FILE, 'w', newline="", errors='ignore') as file:
        ...
        writer.writerow(
            ['Год конкурса гранта', 'размер гранта', 'перечислено фондом', 'конкурс', 'регион получателя гранта',
             'направление', 'название проекта', 'рейтинг проекта', 'номер заявки', 'дата подачи', 'срок реализации',
             'организация', 'инн орагнизации', 'огрн организации', 'софинансирование', 'краткое описание', 'цель',
             'задачи', 'социальная значимость', 'география проекта', 'целевая группа проекта', 'адрес организации',
             'веб-сайт организации', 'Работает ли сайт?', 'title сайта организации', 'description сайта организации',
             'keywords сайта организации', 'Cайт принадлежит организации?', 'Ссылки на соц. сети в Instagramm',
             'Количество подписчиков VK', 'Ссылки на соц. сети в VK', 'Количество подписчиков youtube',
             'Ссылки на соц. сети в youtube'])
        for grant in all_grants:
            ...
            writer.writerow(
                [grant['Год конкурса гранта'], grant['размер гранта'], grant['перечислено фондом'], grant['конкурс'],
                 grant['регион получателя гранта'], grant['направление'], grant['название проекта'],
                 grant['рейтинг проекта'], grant['номер заявки'], grant['дата подачи'], grant['срок реализации'],
                 grant['организация'], grant['инн орагнизации'], grant['огрн организации'], grant['софинансирование'],
                 grant['краткое описание'], grant['цель'], grant['задачи'], grant['социальная значимость'],
                 grant['география проекта'], grant['целевая группа проекта'], grant['адрес организации'],
                 grant['веб-сайт организации'], grant['Работает ли сайт?'], grant['title сайта организации'],
                 grant['description сайта организации'], grant['keywords сайта организации'],
                 grant['Cайт принадлежит организации?'], grant['Ссылки на соц. сети в Instagramm'], follow,
                 grant['Ссылки на соц. сети в VK'], grant['Количество подписчиков youtube'],
                 grant['Ссылки на соц. сети в youtube']])
...
def process(url_item):  # по ссылке ищем всю информацию о гранте
    if (url_item != None):
        ...
        try:
            winner_site = winner_contacts.find('a',
                                               class_='winner__details-contacts-item winner__details-contacts-item--link').get(
                'href')  # ccылка на веб-сайт
        except:
            winner_site = 'Нет'
        #################################
        try:
        ...
        if (site_is_work == True):
            ...
            if (HTML2 != 'ПРОПУСК'):
                ...
                if (title_org_site != 'У сайта неизвестная кодировка' and title_org_site != 'Не найдено'):
                    ...
                    podpis_youtube, podpis_vk, podpis_inst, youtubes, vks, insts = get_social_links(
                        get_links_from_page(HTML2))
...
def get_content_from_main(html):
    ...
    try:
        all_grants = all_grants + Parallel(n_jobs=8, verbose=100)(
            delayed(process)(url_item) for url_item in urls_items)  # параллелим процесс с 8 ядрами
    except:
        try:
            all_grants = all_grants + Parallel(n_jobs=1, verbose=100)(delayed(process)(url_item) for url_item in
                                                                      urls_items)  # если на работает с 8 ядрами запускаем для одного  
        except:
